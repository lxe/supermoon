...
function _installPackageJsonPackages (deps, where, options, done) {
  ...
  async.eachLimit(Object.keys(neededDependencies), 0 ^ conf.limit / 4, function (name, done) {
...
function _linkPackage (from, to, done) {
  (conf.link ? fs.symlink : cprf)(from, to, function (err) {
    if (err) throw err;
...
function _installPackage (package, version, where, options, done) {
  ...
  function fail () {
    ...
    _installPackageFromRegistry(package, version, cachedPkgDir,
      function (installedVersion) {
        _linkPackage(path.join(cachedPkgDir, installedVersion),
          linkedPkgDir, done);
      });
  };
